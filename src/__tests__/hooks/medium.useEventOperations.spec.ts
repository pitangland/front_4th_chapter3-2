...
describe.only('반복 간격 설정', () => {
  it('매일 2일 간격으로 반복일정을 설정할 수 있다', () => {
    ...
    act(() => {
      result.current.setDate('2024-05-01'); // 5월 1일 설정
      result.current.setRepeatType('daily'); // 매일 반복
      result.current.setRepeatInterval(2); // 2일 간격
    });
  ...
  it('매주 3주 간격으로 반복일정을 설정할 수 있다', () => {
    const { result } = renderHook(() => useEventForm());

    act(() => {
      result.current.setDate('2024-05-01'); // 5월 1일 설정 (수요일)
      result.current.setRepeatType('weekly'); // 매주 반복
      result.current.setRepeatInterval(3); // 3주 간격
    });

    const nextDate = result.current.calculateNextRepeatDate(result.current.date);
    expect(nextDate).toBe('2024-05-22'); // 3주 후 수요일
  });

  it('매월 2개월 간격으로 반복일정을 설정할 수 있다', () => {
    const { result } = renderHook(() => useEventForm());

    act(() => {
      result.current.setDate('2024-01-31'); // 1월 31일 설정
      result.current.setRepeatType('monthly'); // 매월 반복
      result.current.setRepeatInterval(2); // 2개월 간격
    });
  ...
  it('매년 2년 간격으로 반복일정을 설정할 수 있다', () => {
    const { result } = renderHook(() => useEventForm());

    act(() => {
      result.current.setDate('2024-05-01'); // 2024년 5월 1일 설정
      result.current.setRepeatType('yearly'); // 매년 반복
      result.current.setRepeatInterval(2); // 2년 간격
    });
